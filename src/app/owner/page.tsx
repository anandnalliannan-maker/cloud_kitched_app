...
export default function OwnerPage() {
  ...
  async function updateServiceSubAreaFee(area: ServiceArea, subArea: string, fee: number) {
    const nextSubAreaFees = {
      ...(area.subAreaFees || {}),
      [subArea]: Number.isFinite(fee) ? Math.max(0, fee) : 0,
    };
  ...
  async function saveServiceSubAreaEdit(area: ServiceArea, previousSubArea: string) {
    ...
    const nextSubAreaFees = Object.fromEntries(
      Object.entries(area.subAreaFees || {}).map(([key, value]) => [
  ...
  const exportPackingMatrixCsv = (
    ...
    const totalPackColumns = matrix.itemNames.reduce(
      (sum, itemName) => sum + (matrix.itemPackQtyColumns[itemName]?.length || 0),
      0
    );
    if (!matrix.rows.length || !matrix.itemNames.length || !totalPackColumns) {
    ...
    const headers = [
      ...
      ...matrix.itemNames.flatMap((itemName) =>
        (matrix.itemPackQtyColumns[itemName] || []).map(
          (packQty) => `${itemName} - Pack ${packQty}`
        )
      ),
    ];
    const rows = matrix.rows.map((row) => [
      ...
      ...matrix.itemNames.flatMap((itemName) =>
        (matrix.itemPackQtyColumns[itemName] || []).map(
          (packQty) => row.items[itemName]?.[packQty] || "-"
        )
      ),
    ]);
                              ...
                              <tr>
                                ...
                                {activeItemPackingMatrix.itemNames.map((itemName, itemIndex) => (
                                  <th
                                    key={itemName}
                                    colSpan={Math.max(
                                      activeItemPackingMatrix.itemPackQtyColumns[itemName]?.length || 0,
                                      1
                                    )}
                              ...
                              {activeItemPackingMatrix.rows.length === 0 && (
                                <tr>
                                  <td
                                    colSpan={
                                      Math.max(
                                        activeItemPackingMatrix.itemNames.reduce(
                                          (sum, itemName) =>
                                            sum +
                                            (activeItemPackingMatrix.itemPackQtyColumns[itemName]?.length || 0),
                                          0
                              ...
                              <tr>
                                ...
                                {pastItemPackingMatrix.itemNames.map((itemName, itemIndex) => (
                                  <th
                                    key={itemName}
                                    colSpan={Math.max(
                                      pastItemPackingMatrix.itemPackQtyColumns[itemName]?.length || 0,
                                      1
                                    )}
                                    className={`packing-group-header packing-group-${itemIndex % 5}`}
                                  >
                            ...
                            <tbody>
                              {pastItemPackingMatrix.rows.length === 0 && (
                                <tr>
                                  <td
                                    colSpan={Math.max(
                                      pastItemPackingMatrix.itemNames.reduce(
                                        (sum, itemName) =>
                                          sum +
                                          (pastItemPackingMatrix.itemPackQtyColumns[itemName]?.length || 0),
                                        0
                              ...
                              <tr>
                                <td colSpan={5} className="owner-assignment-editor-cell">
                                  <div className="owner-assignment-editor">
                                    <strong>{area.name}</strong>
                                    <small className="payments-subtext">
                                      Delivery fee is managed only at sub-area level.
                                    </small>
                                    <div className="table-scroll">
                                      <table className="payments-table payments-table-compact owner-assignment-table">
                                        <thead>
                                        ...
                                        <tbody>
                                          {Array.from(new Set([...mappedSubAreas, ...savedCustomSubAreas])).map((subArea) => {
                                            const editKey = `${area.id}::${subArea}`;
                                            const isEditing = editingAreaSubAreaKey === editKey;
                                            const isCustom = savedCustomSubAreas.includes(subArea);
                                            return (
                                              <tr key={editKey}>
                                                <td>
                                                  {isEditing ? (
                                                ...
                                                <td>
                                                  <input
                                                    ...
                                                    min="0"
                                                    step="1"
                                                    defaultValue={String(Number(area.subAreaFees?.[subArea] || 0))}
                                                    onBlur={(e) =>
                                                      updateServiceSubAreaFee(area, subArea, Number(e.target.value || 0))
                                                    }
                                                    style={{ width: 140 }}
                                                  />
                                                </td>
                                                  ...
                                                  {isCustom ? (
                                                    isEditing ? (
                                                      <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                                                        <button className="btn secondary btn-compact" onClick={() => saveServiceSubAreaEdit(area, subArea)}>
                                                          Save
                                                        </button>
                                                        <button className="btn secondary btn-compact" onClick={() => setEditingAreaSubAreaKey(null)}>
                                                          Close
                                                        </button>
                                                      </div>
                                                    ) : (
                                                      <button className="btn secondary btn-compact" onClick={() => openServiceSubAreaEdit(area, subArea)}>
                                                        Edit
                                                      </button>
                                                    )
                                                  ) : (
                                                    <small className="payments-subtext">-</small>
                                                  )}
                                                </td>
                                              </tr>
                                            );
                                    ...
                                    <div className="row">
                                      <input
                                        ...
                                        style={{ flex: 1 }}
                                      />
                                      <button className="btn secondary btn-compact" onClick={() => addServiceSubArea(area)}>
                                        Add Sub Area
                                      </button>
                                      <button className="btn secondary btn-compact" onClick={() => setOpenManagedAreaId(null)}>
                                        Close
                                      </button>
                                    </div>
                                  </div>
